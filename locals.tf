locals {
  ...
  cloudwatch_log_group_external_name = var.cloudwatch_log_group_external_name ? var.cloudwatch_log_group_external_name : aws_cloudwatch_log_group.mlflow.0.name
  tags = merge(
    {
